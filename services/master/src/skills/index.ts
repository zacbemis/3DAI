...
function parseTechniques(content: string): ParsedTechnique[] {
  ...
  while ((match = regex.exec(content)) !== null) {
    techniques.push({ name: match[1].trim(), keywords: match[2].split(',').map(k => k.trim().toLowerCase()).filter(k => k.length > 0), content: match[0] });
  }
  return techniques;
}
...
export function getSkillsPrompt(description: string): string {
  ...
  let matched = techniques.filter(t => t.keywords.some(kw => keywordMatches(descLower, kw)));
  ...
  const sections = [...matched.map(t => t.content), rules];
